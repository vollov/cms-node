...
const authorization = async (req, res, next) => {
	...
	if (publicAPI(method, path)) {
		log.debug(`authorization, pulbic path=${path}, method=${method}`)
		next()
	} else {
		...
		if (!token) {
			...
			try {
				const user = await jwt.verify(jwt_token, JWT_SECRET)
				log.debug(
					`authorization success, user=${user}, req.path=${req.originalUrl}`
				)
				req.user = user
				next()
			} catch (err) {
